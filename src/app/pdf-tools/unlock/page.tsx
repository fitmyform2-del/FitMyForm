...
import { unlockPdf } from '@/lib/pdf/pdfTools';
import { Unlock, Upload, Download, RefreshCw, ArrowLeft, CheckCircle2, FileText } from 'lucide-react';
...
export default function UnlockPdfPage() {
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      setError('Failed to unlock PDF. Please check if password is correct.');
    } finally {
    ...
    <div className="min-h-screen bg-[#0b0f19] text-gray-100 flex flex-col">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8 space-y-8">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-3xl font-black text-white">Unlock PDF Document</h1>
          <p className="text-xs text-gray-400">Remove password protection and security restrictions from your PDF file.</p>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 space-y-6">
          {!file ? (
            <div className="border-2 border-dashed border-gray-800 hover:border-blue-500/60 rounded-2xl p-8 text-center space-y-3 bg-gray-950/50">
              ...
              <p className="text-sm font-bold text-gray-300">Upload Password-Protected PDF</p>
              <input type="file" accept="application/pdf" onChange={handleFileChange} id="pdf-unlock-input" className="hidden" />
              <label
                htmlFor="pdf-unlock-input"
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-950 p-4 rounded-2xl border border-gray-800 space-y-2">
                <label className="text-xs font-bold text-gray-400">Enter PDF Password (if prompt required)</label>
                <input
                  type="password"
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  placeholder="Document password..."
                  className="w-full bg-gray-900 border border-gray-800 rounded-xl px-4 py-2.5 text-xs text-white placeholder-gray-500"
                />
              </div>
              ...
              >
                {isProcessing ? <RefreshCw className="w-5 h-5 animate-spin" /> : <Unlock className="w-5 h-5" />}
              ...
              {unlockedUrl && (
                <div className="p-6 bg-emerald-950/60 border border-emerald-500/40 rounded-2xl text-center space-y-3">
                  <div className="flex items-center justify-center gap-2 text-emerald-400 font-bold text-sm">
                    <CheckCircle2 className="w-5 h-5" /> PDF Unlocked Successfully!
                  </div>
                  <a
                    href={unlockedUrl}
