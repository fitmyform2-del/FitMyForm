...
import { redactPdf, RedactionArea } from '@/lib/pdf/pdfTools';
import { EyeOff, Upload, Download, RefreshCw, ArrowLeft, CheckCircle2, FileText, Plus, Trash2 } from 'lucide-react';

export default function RedactPdfPage() {
  const [file, setFile] = useState<File | null>(null);
  const [redactions, setRedactions] = useState<RedactionArea[]>([
    { pageIndex: 0, xRatio: 0.1, yRatio: 0.1, wRatio: 0.8, hRatio: 0.05 },
  ]);
  ...
  const handleAddRedaction = () => {
    setRedactions((prev) => [
      ...prev,
      { pageIndex: 0, xRatio: 0.2, yRatio: 0.2 + prev.length * 0.08, wRatio: 0.6, hRatio: 0.05 },
    ]);
  };
    ...
    <div className="min-h-screen bg-[#0b0f19] text-gray-100 flex flex-col">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8 space-y-8">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-3xl font-black text-white">Redact Sensitive PDF Info</h1>
          <p className="text-xs text-gray-400">Permanently black out Aadhaar numbers, phone numbers, signatures, and private content.</p>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 space-y-6">
          {!file ? (
            <div className="border-2 border-dashed border-gray-800 hover:border-blue-500/60 rounded-2xl p-8 text-center space-y-3 bg-gray-950/50">
              ...
              <p className="text-sm font-bold text-gray-300">Upload PDF to Redact</p>
              <input type="file" accept="application/pdf" onChange={handleFileChange} id="pdf-redact-input" className="hidden" />
              <label
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3 bg-gray-950 p-5 rounded-2xl border border-gray-800">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-gray-300">Active Redaction Blackout Blocks ({redactions.length})</span>
                  <button
                  ...
                  >
                    <Plus className="w-3.5 h-3.5 text-blue-400" /> Add Blackout Block
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  {redactions.map((red, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-gray-900 p-3 rounded-xl border border-gray-800 text-xs">
                      <span className="font-bold text-gray-300">Block #{idx + 1} (Page 1 Redaction Box)</span>
                      <button
                        onClick={() => setRedactions(redactions.filter((_, i) => i !== idx))}
              ...
              >
                {isProcessing ? <RefreshCw className="w-5 h-5 animate-spin" /> : <EyeOff className="w-5 h-5" />}
                <span>Burn Permanent Redactions to PDF</span>
              </button>

              {redactedUrl && (
                <div className="p-6 bg-emerald-950/60 border border-emerald-500/40 rounded-2xl text-center space-y-3">
                  <div className="flex items-center justify-center gap-2 text-emerald-400 font-bold text-sm">
                    <CheckCircle2 className="w-5 h-5" /> Redactions Applied Permanently!
                  </div>
                  <a
                    href={redactedUrl}
